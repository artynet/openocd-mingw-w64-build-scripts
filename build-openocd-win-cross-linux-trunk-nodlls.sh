...
then
  ...
  # For regular read/only access, use the git url.
  git clone https://github.com/arduino-org/openocd-official-mirror.git -b master openocd-arduino
...
# renaming folder
mv ${OPENOCD_INSTALL_FOLDER} ${OPENOCD_WORKING_FOLDER}/OpenOCD-0.10.x-dev-win${TARGET_BITS}

cd ${OPENOCD_WORKING_FOLDER}

tar cfvj OpenOCD-0.10.x-dev-win${TARGET_BITS}-${stag}.tar.bz2 OpenOCD-0.10.x-dev-win${TARGET_BITS}/

# Removing build folder
rm -rf ${OPENOCD_WORKING_FOLDER}/OpenOCD-0.10.x-dev-win${TARGET_BITS}
