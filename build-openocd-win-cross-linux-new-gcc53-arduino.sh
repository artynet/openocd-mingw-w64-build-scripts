...
LIBUSB1="libusb-1.0.21"

# https://github.com/signal11/hidapi/downloads
HIDAPI="hidapi-0.8.0-rc1"
...
then
  ...
  then
    ...
    cd "${OPENOCD_GIT_FOLDER}"
    git pull
    git checkout nrf52-test-g19df456
    git submodule update
...
then
  ...
  # For regular read/only access, use the git url.
  git clone https://github.com/arduino-org/openocd-official-mirror.git -b nrf52-test-g19df456 openocd-arduino
...
then
  ...
  "${WGET}" "http://download.arduino.org/tools/${LIBUSB1_ARCHIVE}" \
...
then
  ...
  cd "${OPENOCD_DOWNLOAD_FOLDER}"
  "${WGET}" "http://download.arduino.org/tools/${LIBUSB_W32_ARCHIVE}" \
...
then
  ...
  cd "${OPENOCD_DOWNLOAD_FOLDER}"
  "${WGET}" "http://download.arduino.org/tools/${LIBFTDI_ARCHIVE}" \
...
then
  ...
  cd "${OPENOCD_DOWNLOAD_FOLDER}"
  "${WGET}" "http://download.arduino.org/tools/${HIDAPI_ARCHIVE}" \
...
# renaming folder
mv ${OPENOCD_INSTALL_FOLDER} ${OPENOCD_WORKING_FOLDER}/OpenOCD-0.10.x-dev-arduino

cd ${OPENOCD_WORKING_FOLDER}

tar cfvj OpenOCD-0.10.x-dev-arduino.org-win${TARGET_BITS}-${stag}-nrf52.tar.bz2 OpenOCD-0.10.x-dev-arduino/

# Removing build folder
rm -rf ${OPENOCD_WORKING_FOLDER}/OpenOCD-0.10.x-dev-arduino
